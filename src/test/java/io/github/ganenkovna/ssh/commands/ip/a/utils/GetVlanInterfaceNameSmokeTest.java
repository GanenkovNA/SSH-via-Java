...
/**
 * Smoke-тест метода {@link IpAddrTestSupport#getVlanInterfaceName(String, int)}.
 *
...
public class GetVlanInterfaceNameSmokeTest extends TestBase {
  ...
  private static final String expectedName = testInterface + "." + validVlanId;

  /**
   * Проверяет, что {@link IpAddrTestSupport#getVlanInterfaceName(String, int)}
   * возвращает имя VLAN-интерфейса в корректном формате.
   */
  @Test
  @DisplayName("Проверка генерации имени VLAN интерфейса для `ip a`")
  public void shouldReturnTrue(){
    String result = IpAddrTestSupport.getVlanInterfaceName(testInterface, validVlanId);
